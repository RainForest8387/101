...
в read-only проверке п. 1.1. Если ZK закрыт SASL/ACL, потребуется свой JAAS:
...
Если доступа к ZK нет — не блокирует: всё, что проверяется через ZK, доступно
и через `kafka-broker-api-versions.sh` / `kafka-configs.sh`.
...
# Детали регистрации каждого брокера (endpoints, версия)
...
### 1.4. Кластер здоров ДО начала работ
...
### 1.5. Остатки троттлинга от прошлых работ
