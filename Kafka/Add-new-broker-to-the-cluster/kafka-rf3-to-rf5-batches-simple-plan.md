...
* **Порядок действий:** диагностика кластера с сохранением отчёта → пачки по 10 топиков → план из штатного
`--generate`, дополненный до RF=5 → выполнение пачек по одной под троттлингом

---

## 0. Краткое описание необходимости работ
...
### Что фактически происходит при добавлении реплики
...
П. 2 причина возможной проблемы: **неконтролируемое копирование нагружает сеть и диски и увеличивает
...
| Пачки по 10 топиков, от мелких к крупным | Ограничивает объём одновременного копирования; первая пачка — пробная |
| Размещение предлагает штатный `--generate` | Не изобретаем велосипед, лидеры распределяются по всем брокерам штатным алгоритмом |
...
> [!Обратить внимание]
> Если после выполнения пробной пачки у всех партиций из пачки будет порядок  `{1,2,3,4,5}` то стоит задуматься о самописном скрипте для того чтобы не у всех партиций лидер реплика оказалась на брокере1
...
### Последстия RF=5, которые нужно обдумать до начала работ
...
| Нагрузка на диски брокеров 1–3 | **Не уменьшится.** Их данные остаются на месте. Разгружается только лидерство (запись от продюсеров, чтение консьюмерами) | Если цель — освободить диски брокеров 1–3, нужен другой подход: перенос части реплик на 4 и 5 при RF=3 (в этой инсттрукции пока не рассматриваем) |
...
| Задержка `acks=all` | Лидер ждёт подтверждения от всех реплик ISR, теперь их 5. p99 записи может вырасти, особенно если один брокер медленнее остальных | Сравнить p99 `Produce TotalTimeMs` до и после пробной пачки |
...
Если что-то из этого неприемлемо или вызывает опасения , работы не начинать и пересмотреть целевой RF.

### Схема работ

```
Этап 1  diagnose.sh before     → cluster-report-before.md  + REPORT.md (итоговый файл)
            │  ничего не делаем, если что-то не так с текущим состоянием кластера
...
Этап 4  run-batch.sh 01        → пробная пачка (самые мелкие топики), проверка влияния
...
Все команды рассчитаны на две переменные окружения, доступ к кластеру и набор параметров плана:

| Переменная | Содержимое |
|---|---|
| `$broker` | все брокеры через запятую, `host:port,host:port,…` — значение для `--bootstrap-server` |
| `$config` | properties-файл для SASL-аутентификацией суперпользователя admin |

```bash
# доступ к кластеру
broker=broker1:9094,broker2:9094,broker3:9094,broker4:9094,broker5:9094
...
MAX_PARTS=300              # максимум партиций в пачке (чтобы крупные топики не набивались в одну пачку)
LARGE_GIB=100              # топик больше этого объёма (GiB) идёт отдельной пачкой
THROTTLE=31457280          # 30 МБ/с на брокер, решение о используемом значение принять на этапе 3

NEW_BROKERS=4,5            # id новых (пока пустых) брокеров

# необязательные
ZK=zk1:2181                             # для проверки /brokers/ids и /controller; пусто, если нет доступа
DF_HOSTS="broker1 broker2 broker3 broker4 broker5"   # для df по ssh; пусто, если будет использвоаться ssh
...
> `$broker` должен перечислять **все 5** брокеров, без пробелов. Тогда команды отработают, если
> какой-то брокер окажется недоступен.
> Утилиты Kafka (`kafka-topics.sh` и т. д.) должны быть в `PATH` пользователя, то есть проверить что `PATH` содержит путь к бинарникам kafka . Нужен `python3` (3.6+),
...
ls -l "$config"          # должны быть права 600/640
...
# ожидаем 5 строк: (id: 1 …) … (id: 5 …); ошибка аутентификации — останавливаем работы
...
Поскольку в кластере включены ACL, пользователю из `$config` нужны:
...
Нехватка прав означает что ошибки `ClusterAuthorizationException` / `TopicAuthorizationException`
будут уже на этапе выполнения команд с флагом `--execute`, поэтому эти проблемы нужно решить до начала каких либо действий

**ZooKeeper** используется только для чтения (`/brokers/ids`, `/controller`) в диагностике.
Если доступа к ZK нет или нет желания использовать необходимо  оставить поле `ZK` пустым. Проводимым работам это не помешает.

### 1.3. Готовность брокеров 4 и 5

**Регистрация и адреса.** Брокеры 4 и 5 должны быть видны AdminClient (п. 1.2).

```bash
for id in 4 5; do zookeeper-shell.sh $ZK get /brokers/ids/$id | tail -1; done
```
если зукипер не используем то пропускаем этот пункт

**Сетевая связность в обе стороны.** Проверяем доступ по TCP до listener-порта
по адресам из `advertised.listeners`. Запустить на **каждом** брокере:

```bash
for h in broker1 broker2 broker3 broker4 broker5; do
  timeout 3 bash -c "</dev/tcp/$h/9094" && echo "OK   $h:9094" || echo "FAIL $h:9094"
done
...
`unclean.leader.election.enable` выдаст вердикт «НЕ ЗДОРОВ». Например, если на новом брокере
...
Снять до начала работ, чтобы было с чем сравнивать после пробной пачки:
...
| `kafka.server:type=BrokerTopicMetrics,name=BytesInPerSec` / `BytesOutPerSec` | штатная нагрузка, сеть |
...
| Лаг критичных консьюмер-групп | влияние на бизнес-процессы |

### 1.5. Организационные вопросы

- [ ] Согласовано тех.окно работ (пачки можно выполнять под нагрузкой, но начинать лучше вне пиковой нагрузки).
...
Примерная структура рабочей диреткории к концу тех.работ:
...
├── diag-before/, diag-after/   # промежуточные выводы утилит
└── batches/
...
Скрипт **только читает файлы** в рабочем каталоге и на кластер никакого влияения не оказывает. Все обращения к Kafka
...
def cmd_plan(a):
    ...
    cand.sort()   # от мелких к крупным: первая пачка — пробная
...
На этом этапе состояние кластера **не меняется**, выполнется только диагностика.

### 2.1. Скрипт диагностики

Скрипт собирает выводы утилит в `diag-<этап>/`, строит отчёт `cluster-report-<этап>.md` и
...
| 3. Топики, партиции, реплики | число топиков (пользовательских/служебных), партиций, реплик, объём данных без учёта реплик и с репликами | Зафиксировать как основу для сравнения «после» |
...
**Предупреждения** (`[WARN]`) работы не блокируют, но решение по каждому нужно зафиксировать в `REPORT.md`:
...
1. `rf5tool.py plan` отбирает пользовательские топики (без `__*`) с RF < 5, сортирует их **по
   ...
   Первая пачка содержит самые мелкие топики и является пробной.
...
весь каталог **за пределы кластера** (другой раздел, себе на локальный раздел, сетевую шару и т.д):

```bash
tar czf ~/kafka-rf5-plan-$(date +%Y%m%d-%H%M).tgz -C "$WORKDIR" batches REPORT.md cluster-report-before.md
```

> Не перезапускать `prepare-batches.sh` после начала выполнения пачек. `--generate` снимет
...
а снижать его, когда задержки уже выросли, накладнее.
...
Троттл выставляется **через `--execute --throttle`** (на брокерах `*.replication.throttled.rate`,
...
Посмотреть `ReplicationBytesInPerSec` на брокерах 4 и 5. Если скорость заметно ниже `THROTTLE`,
...
* забытый троттл на топике или брокере с меньшим значением (п. 7.1);
...
> **Важно.** Во время ожидания использовать только `--verify --preserve-throttles`. Обычный `--verify`
> снимает троттлы, как только сочтёт реассайн завершённым. Скрипт вызывает его только на шаге 4.

### 5.3. Порядок выполнения

**Пробный запуск — пачка 01** (самые мелкие топики):

```bash
./run-batch.sh 01
```

После пробного запуска, перед следующими пачками:
...
> но лучше не испытывать его и судьбу.
...
50 партиций, в каждой лежит координатор для части консьюмер-групп. Делим не по топикам, а по
**10 партиций** в пачке:
...
Каждая пачка снимает свои троттлы сама (шаг 4 скрипта). Проверить — ничего не должно остаться:
...
Для RF=5 обычно рекомендуется `min.insync.replicas=3`. Менять **после** завершения всех пачек и
согласовать с тимлидами команд разработки, использующих `acks=all`:
...
Это надо согласовать с тимлидами команд разработки.
...
При откате реплики на 4 и 5 просто удаляются.
...
- [ ] Последствия RF=5 согласованы: место, сеть, `acks=all` (п. 0)
...
**Этап 3–4 — выполнение**
- [ ] `THROTTLE` выбран
- [ ] Пробный запуск (пачка `01`) выполнен, метрики в норме
