#!/bin/bash
#
# systemd-units-watcher.sh — проверка состояния и автозапуск systemd-юнитов Kafka-стека.
#
# Что делает:
...
#
# ВАЖНО: по умолчанию скрипт работает в режиме DRY-RUN. Для запуска в рабочем режиме использовать флаг --apply.
...
# (в этом случае необходимо будет еще добавить ротацию логов)
#
...
#                         Суффикс .service можно не писать. Отменяет список по умолчанию.
