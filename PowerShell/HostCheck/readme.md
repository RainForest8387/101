...
```powershell
powershell   .\Test-SshConnection.ps -HostsFile .\hosts.txt
```

2.Проверка через массив хостов прямо в команде:
```
powershell   .\Test-SshConnection.ps -HostList "10.0.0.1","server2.local" -Port 2222
```
...
powershell   Install-Module -Name Posh-SSH -Scope CurrentUser
   $cred = Get-Credential
   .\Test-SshConnection.ps -HostsFile .\hosts.txt -Credential $cred
```
Функционал:
